...
// Extended topic passages with more variety
const topicPassages = {
    technology: [
...
// Extended word list with more variety
const commonWords = [
...
// Generate random words with optional punctuation and numbers
function generateRandomWords(count, includePunctuation = false, includeNumbers = false) {
    const words = [];
    ...
    // Add final punctuation if enabled
...
function getPassageByTopic(topic) {
...
router.post('/generate', async (req, res) => {
    try {
        const { topic, wordCount = 50, punctuation, numbers } = req.body;

        if (topic) {
            let passage = getPassageByTopic(topic);
            res.json({
                text: passage,
                topic,
                source: 'curated',
            ...
            const text = generateRandomWords(wordCount, punctuation, numbers);
            res.json({
                text,
                source: 'random',
                wordCount
...
router.get('/words', (req, res) => {
    try {
        ...
        const includePunctuation = req.query.punctuation === 'true';
        const includeNumbers = req.query.numbers === 'true';
        const text = generateRandomWords(Math.min(count, 200), includePunctuation, includeNumbers);
...
router.get('/quote', (req, res) => {
    try {
        ...
        const passage = allPassages[Math.floor(Math.random() * allPassages.length)];
        res.json({
            text: passage,
            wordCount: passage.split(/\s+/).length
        });
...
router.get('/topics', (req, res) => {
    res.json({
        topics: Object.keys(topicPassages).map(t => t.charAt(0).toUpperCase() + t.slice(1))
    });
});
